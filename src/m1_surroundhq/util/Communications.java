...
public class Communications {
    ...
    private final int OTHER_X = MAP_SIZE - 6, OTHER_Y = MAP_SIZE - 5;  // for mapIdx=0, will hold enemy HQ location
...
//        if (uc.getRound() >= 1450) uc.println("start reportEnemySighting " + uc.getEnergyUsed());
        ...
        while (idx >= 0 && enemies[idx] == null) --idx;
        if (idx >= 0 && enemies[idx] != null && enemies[idx].getType() == UnitType.HQ) {
            uc.write(MAP_OFFSET + OTHER_X, enemies[idx].getLocation().x);
            uc.write(MAP_OFFSET + OTHER_Y, enemies[idx].getLocation().y);
            createDistanceMapIfNotExists(enemies[idx].getLocation(), 0);
            --idx;
        }
        while (idx >= 0 && enemies[idx] == null) --idx;
        if (idx < 0) return;
...
//        uc.println("start grass " + uc.getEnergyUsed());
        for (int i = grass.length - 1; i >= 0 && uc.getRound() == currentRound; --i) {
            ...
            if (readMapLocation(0, x, y) < PASSABLE && !getSelfHQLocation().isEqual(grass[i])) {
                ...
                for (int m = uc.read(MAP_OFFSET + DISTANCE_MAP_COUNT); m > 0; --m) {
                    if (readMapLocation(m, x, y) == INF) {
                        ...
                        if (dist < readMapLocation(m, x, y)) {
//                            uc.println("new grass m, x, y, dist: " + m + " " + x + " " + y + " " + dist);
...
//        uc.println("end grass " + uc.getEnergyUsed());
    ...
    public Location getEnemyHqLocation() {
        if (uc.read(MAP_OFFSET + OTHER_X) == UNINITIALIZED) return new Location(-5000, -5000);  // far away but within int range for distanceSquared
        return new Location(uc.read(MAP_OFFSET + OTHER_X), uc.read(MAP_OFFSET + OTHER_Y));
    }
...
//            uc.println("map, dist, x, y: " + mapIdx + " " + dist + " " + x + " " + y);
...
//        uc.println("checking map, x, y, dist, dc, read0, readmap: " + mapIdx + " " + newX + " " + newY + " " + dist + " " + distChange + " " + readMapLocation(0, newX, newY) + " " + readMapLocation(mapIdx, newX, newY));
        if (readMapLocation(0, newX, newY) != UNINITIALIZED) {
            if (infIfZero(readMapLocation(mapIdx, newX, newY)) > dist + distChange) {
//                uc.println("adding map, x, y, dist: " + mapIdx + " " + newX + " " + newY + " " + (dist + distChange));
    ...
    private void createDistanceMapIfNotExists(Location externalLoc, int buffer) {
        ...
        if (readMapLocation(0, internalX, internalY) < PASSABLE) return;

        int n = uc.read(MAP_OFFSET + DISTANCE_MAP_COUNT);
        ...
        n = uc.read(MAP_OFFSET + DISTANCE_MAP_COUNT);  // make more robust against bytecode issues
        if (n >= MAX_MAP_COUNT) return;
